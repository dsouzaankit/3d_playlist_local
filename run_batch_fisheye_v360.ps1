# Batch fisheye v360 workflow: same prepare + pass-2 chase as context menu, one clip at a time.
...
if ($env:FISHEYE_BATCH_USING_SYNCED -ne '1') {
    $syncedBatch = Join-Path $playlistLocal 'run_batch_fisheye_v360.ps1'
...
$fisheyeSegmentOutDir = if (Get-Command Get-DlnaSegmentOutputDirectory -ErrorAction SilentlyContinue) {
    ...
    Join-Path 'F:\f1_media\3d_fullsbs_trans' 'fisheye'
}
...
$mutexName = 'Global\V360FisheyeBatch'
...
try {
    ...
    while ((Get-Date) -lt $mutexDeadline) {
        try {
            ...
            Write-Warning 'Previous fisheye batch left an abandoned mutex (window closed); taking ownership and continuing.'
            $mutexAcquired = $true
            break
        }
        $others = @(Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" -ErrorAction SilentlyContinue |
            Where-Object { $_.ProcessId -ne $PID -and $_.CommandLine -match 'run_batch_fisheye_v360|Run-V360PrepareFisheye' })
    ...
    if ($DryRun) {
        ...
        foreach ($line in $mediaLines) {
            ...
            if (Test-SkipStreamTo3DMediaName -FileName $baseName -FullPath $mediaFull) {
                ...
                Write-Host "[DryRun][Clip] prepare -AutoChaseTranscode -ChaseSync -> wait ${InterClipWaitSec}s"
                Write-Host "  media: $mediaFull"
                Write-Host "  mezz:  $frag"
                Write-Host "  avs:   $avs"
                Write-Host ("  DLNA:  {0}\3d_op_%02d_LR_180_FISHEYE.mkv" -f $fisheyeSegmentOutDir)
            }
            if ($dryRunIdx -eq 0) {
